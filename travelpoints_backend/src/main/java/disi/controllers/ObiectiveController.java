...
import disi.dtos.ResetDataDTO;
import disi.dtos.UserDTO;
import disi.entities.Obiective;
import disi.repositories.ObiectiveRepository;
import disi.services.ObiectiveService;
import disi.services.UserService;
import disi.services.WishService;
...
public class ObiectiveController {
    ...
    //vizualizareDescriereAudio
    @GetMapping(value = "/adminboard/objectives/audio/{id}")
    public ResponseEntity<String> vizualizareAudio(@PathVariable("id") int obiectivId) {
        ObiectiveDTO dto=obiectiveService.vizualizareText(obiectivId);

        return new ResponseEntity<>(dto.getDescriere_audio(),HttpStatus.OK);
    }
